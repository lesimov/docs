...
<Steps>
  ...
  <Step title="Unzip downloaded files">
    1. Create a new folder named `[dusa]`
    2. Unzip all content inside `[dusa]` folder
  </Step>
  <Step title="Add to server start">
    1. Open your `server.cfg`
    2. Add the following line\
       `ensure [dusa]`
  </Step>
  <Step stepNumber={4} title="Add items">
    1. Navigate into following path

       `[dusa]/dusa_tablet/INSTALL`
    2. If you using `ox_inventory`\
       2.1 Open `ox_inventory_items.txt` , copy content\
       2.2 Navigate into `ox_inventory/data/items.lua` and paste to bottom\
       2.3 Open `illegal_network_items_ox_inventory.txt` , copy content\
       2.4 Open `ox_inventory/data/items.lua` and paste them too
    3. If you using `other inventories`\
       3.1 Add the items that included at`qb_core_items.txt` and `illegal_network_items_qb_core.txt` to your framework or your core. It depends on which inventory or core is this.
  </Step>
  <Step stepNumber={5} title="Add item images">
    1. Navigate into following path\
  ...
  <Step stepNumber={6} title="IF YOU USING BUNDLED VERSION">
    1. Navigate into following path
    ...
    5. Paste to your `ox_inventory items.lua`. If you dont using ox, apply same step depends on your inventory or core
  </Step>
</Steps>
